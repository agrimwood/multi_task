...
def us_generator(dataframe, img_path, msk_path, batch_size):
    ...
    while True:
        ...
        while b < batch_size:
            ...
            if int(images_list[i][-7:-4]) > 9:
                ...
                for n in range(10):
                    # augment images
                    image1 = (krs.load_img(filelist[n],color_mode='rgb', target_size=(512, 512)))
                    image1.rotate(rnd_angle)
                    image1.transform(image1.size, Image.AFFINE,(1,0,rnd_x,0,1,rnd_y))
                    # augment masks
                    if n==9:
                        mask1 = (krs.load_img(masklist[n],color_mode='grayscale', target_size=(512, 512)))
